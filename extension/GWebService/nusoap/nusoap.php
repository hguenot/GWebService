...
class nusoap_server extends nusoap_base {
	...
	/**
	 * Provider for webservice execution.
	 * @var object
	 * @access public
	 */
	var $provider = false;
	...
	*/
	function invoke_method() {
		...
		// instanciate provider if needed
		$providerClass = false;
		$provider = false;
		if ($this->provider !== false && $this->provider != '' && $this->provider != null) {
			if (is_string($this->provider)) {
				$providerClass = $this->provider;
				$this->debug('in invoke_method, instanciate new provider : ' . $providerClass);
				$this->provider = new $providerClass();
			} else if (is_object($this->provider)) {
				$providerClass = get_class($this->provider);
				$this->debug('in invoke_method, using current provider : ' . $providerClass);
		...
		if (!function_exists('call_user_func_array')) {
			if ($class == '') {
				...
				if ($delim == '..') {
					...
					if ($providerClass === false)
						...
						$funcCall = "\$this->methodreturn = ".$providerClass."::".$method."(";
				} else {
					...
					if ($provider === false)
						...
						$funcCall = $instname." = \$this->provider; ";
			...
			if ($class == '') {
				...
				if ($provider === false)
					...
					$call_arg = array($provider, "$this->methodname");	// straight assignment changes $this->methodname to lower case after call_user_func_array()
			} elseif ($delim == '..') {
				$this->debug('in invoke_method, calling class method using call_user_func_array()');
				if ($providerClass === false)
					$call_arg = array ($class, $method);
				else
					$call_arg = array ($providerClass, $method);
			} else {
				$this->debug('in invoke_method, calling instance method using call_user_func_array()');
				// GWebService adaptation
				if ($provider === false)
					$instance = new $class ();
				else
	...
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if(!xml_parse($this->parser,$xml,true)){
				...
				// get final value
				if (isset($this->message[$this->root_struct]) && isset($this->message[$this->root_struct]['result']))
